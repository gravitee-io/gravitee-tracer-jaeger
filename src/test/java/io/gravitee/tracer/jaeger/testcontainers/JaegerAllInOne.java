 ...
 */
package io.gravitee.tracer.jaeger.testcontainers;
...
public class JaegerAllInOne extends GenericContainer<JaegerAllInOne> {
    ...
    public JaegerAllInOne(DockerImageName imageName) {
        super(imageName);
        setWaitStrategy(Wait.forHttp("/").forPort(JAEGER_ADMIN_PORT).forStatusCode(200));
        withExposedPorts(JAEGER_ADMIN_PORT, JAEGER_COLLECTOR_GRPC_PORT, JAEGER_FRONTEND_PORT);
    }
